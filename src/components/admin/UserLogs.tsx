    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm">Last 6 months &bull; {logs.length} entries loaded</p>
        </div>
        <button
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        <div className="flex items-center gap-3">
          ...
          />
          {searchEmail && (
            <button onClick={() => setSearchEmail('')} className="text-gray-400 hover:text-gray-600">✕</button>
          )}
        </div>
          ...
          <div className="block sm:hidden">
            {filteredLogs.map(log => (
              <div key={log.id} className="p-4 border-b border-gray-100 last:border-0">
                ...
                <p className="text-sm font-medium text-gray-700 truncate">{log.userEmail}</p>
                <p className="text-xs text-gray-500">{log.page}</p>
              ...
              <tbody>
                {filteredLogs.map((log, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-gray-700 text-sm">{log.userEmail}</td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center gap-1 text-xs px-2 py-1 rounded-full font-medium ${getActionColor(log.action)}`}>
                        {log.action.replace('_', ' ')}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-gray-500 text-sm font-mono">{log.page}</td>
          ...
          {hasMore && (
            <div className="p-4 border-t border-gray-100 text-center">
              <button
                ...
                className="px-6 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-sm font-medium transition-colors disabled:opacity-50"
              >
                {loadingMore ? (
